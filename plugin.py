...
<plugin key="AMZZ" name="AC Zone control in multizone central duct air system" author="Erwanweb" version="0.0.2" externallink="https://github.com/Erwanweb/AMZZ.git">
    ...
    <params>
        ...
        <param field="Mode5" label="hysterisis, Delta Max(in tenth of degre)" width="200px" required="true" default="5,10"/>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if 3 not in Devices:
            Domoticz.Device(Name="Zone heating mode", Unit=3, TypeName="Switch", Image=9, Used=1).Create()
            devicecreated.append(deviceparam(3, 0, ""))  # default is Off
        if 4 not in Devices:
            Domoticz.Device(Name="Main duct heating mode", Unit=4, TypeName="Switch", Image=9, Used=1).Create()
            devicecreated.append(deviceparam(4, 0, ""))  # default is Off
        if 5 not in Devices:
            Domoticz.Device(Name="Setpoint", Unit=5, Type=242, Subtype=1, Used=1).Create() # default is 21
        ...
        if 7 not in Devices:
            Domoticz.Device(Name="Air power Request", Unit=7, TypeName="Switch", Image=9, Used=1).Create()
        ...
        Domoticz.Debug("Heaters = {}".format(self.Airvalve))
    ...
    def onCommand(self, Unit, Command, Level, Color):
        ...
        if Unit == 1:  # zone control
            ...
            Devices[2].Update(nValue = Devices[1].nvalue,sValue = Devices[2].sValue)
            self.onHeartbeat()
        ...
        if Unit in (3, 4, 5, 6, 7): # tout le reste
            self.onHeartbeat()
    ...
    def onHeartbeat(self):
        ...
        self.setpoint = (Devices[5].sValue)
        ...
        if Devices[1].sValue == "0":  # Thermostat is off
            ...
            if Devices[2].sValue == "10":  # Mode is COOLING
                if not Devices[3].nValue == 0:
                    Devices[3].Update(nValue = 0,sValue = Devices[3].sValue)
                if self.intemp > (float(Devices[5].sValue) - (self.hyste / 10)) : # we have a cooling request
                ...
                if self.intemp < (float(Devices[5].sValue) - (self.hyste / 10)) : # we have a heating request
                    self.powerrequest = True
                    if Devices[7].nValue == 0:
                        Devices[7].Update(nValue = 1,sValue = Devices[7].sValue)
                else:
    ...
    def Mainductmodecontrol(self):
        ...
        if Devices[1].sValue != "0":  # Auto or manual mode is on
             self.Mainductheatmode = False
                ...
                for device in devicesAPI["result"]:  # parse the Heating requester device
                    ...
                    if idx in self.Mainductmode:  # this is one of our Heating requester switch
                        if "Status" in device:
                            ...
                            Domoticz.Debug("Main duct heating mode switch {} currently is '{}'".format(idx,device["Status"]))
                            if device["Status"] == "On":
                                self.Mainductheatmode = True

                        else:
                            Domoticz.Error("Device with idx={} does not seem to be a Main duct heating mode switch !".format(idx))


             # fool proof checking....
             if len(Mainductmodeswitch) == 0:
                Domoticz.Error("none of the devices in the 'Main duct heating mode switch' parameter is a switch... no action !")
                self.Mainductheatmode = False
                self.powerrequest = False
